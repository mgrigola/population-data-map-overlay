...
def translate_shapefile(shapeFile, recFiltFunc, boundingBox, dataProp, newDataProp):
    ...
    with fiona.open(shapeFile+'.shp', 'r') as src:
        ...
        with fiona.open(shapeFile+'.json', 'w', **meta) as sink:
            filtSrc = filter(recFiltFunc, src)
            for rec in filtSrc:
                ...
                if rec['geometry']['type'] != 'Polygon':
                    ...
                    # if polygon has negative space, the first element is the whole, skip the other pieces
                    # if polygona consiss of multiple pieces we include all of those
...
if __name__ == "__main__":
    ...
    shapeFile = "../data/WI_County/cb_2015_us_zcta510_500k"
